...
class ModuleLoader:
    ...
    def load_module(cls, module_path: str):
        ...
        log.debug("Module '{}' loaded.".format(module.__name__))
        return module
...
def _resolve_local_module(module_path):
    ...
    if _is_absolute_import(module_path):
        path = get_absolute_path(module_path)

    elif _is_relative_import(module_path):
        path = get_relative_path(module_path)

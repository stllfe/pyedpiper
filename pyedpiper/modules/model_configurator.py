...
if __name__ == "__main__":
    outputs = [
        {'test_loss': torch.as_tensor(1).float(), 'progress_bar': {'test_acc': torch.as_tensor(0.3).float()}},
        {'test_loss': torch.as_tensor(2).float(), 'progress_bar': {'test_acc': torch.as_tensor(0.6).float()}},
        {'test_loss': torch.as_tensor(0.4).float(), 'progress_bar': {'test_acc': torch.as_tensor(0.3).float()}},
        {'test_loss': torch.as_tensor(0.2).float(), 'progress_bar': {'test_acc': torch.as_tensor(0.1).float()}},
        {'test_loss': torch.as_tensor(0.1).float(), 'progress_bar': {'test_acc': torch.as_tensor(1).float()}}
    ]

    new_dict = average_and_log_metrics(outputs)
    print(new_dict)
    print(torch.as_tensor([1, 2, 0.4, 0.2, 0.1]).mean(), torch.as_tensor([0.3, 0.6, 0.3, 0.1, 1]).mean())
